...
* **Multi-View Filtering:** Integrated logic to isolate "Available" or "Occupied" beds instantly via the Stats Cards.
...
* **Tech:** `FastAPI`, `Pydantic`, `Motor (Async Mongo)`, `MySQL-Connector`.
...
* **Initial Setup:** Run `simulation/generate_history.py` **once** to ensure the "WaitLess" engine has historical data to query before the demo starts.
* **Environment:** Ensure both `mongodb` and `mysql` services are running.
* **Live Demo:** Keep the `sensor_emitter.py` running in a separate terminal during the presentation to show the real-time grid updates and the "WaitLess" engine in action.
